-- Allow managers to create a standalone order for a physiotherapy practice.
...
check (order_type in ('customer', 'physio'));

drop function if exists public.create_admin_order(uuid, jsonb, text, text, text, integer, text);

create function public.create_admin_order(
  p_customer_id uuid,
  p_items jsonb,
...
-- Transactional webshop emails do not belong to internal physiotherapy orders.
