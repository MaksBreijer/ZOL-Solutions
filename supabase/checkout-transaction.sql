...
begin
  ...
  insert into public.orders (
    ...
    subtotal_cents, shipping_cents, tax_cents, total_cents, currency, shipping_address, note, source
  ) values (
    v_customer_id, v_email, v_customer_name, 'open', 'pending', 'unfulfilled',
    v_subtotal, v_shipping, v_tax, v_total, 'EUR',
    jsonb_build_object(
      'street', left(coalesce(p_customer ->> 'street', ''), 180),
      'postal_code', left(coalesce(p_customer ->> 'postal_code', ''), 30),
      'city', left(coalesce(p_customer ->> 'city', ''), 120),
  ...
  values (left(coalesce(nullif(p_session_id, ''), gen_random_uuid()::text), 120), 'order_created', '/checkout/', jsonb_build_object('order_number', v_order_number));
...
revoke all on function public.create_checkout_order(jsonb, jsonb, text, text) from public, anon, authenticated;
grant execute on function public.create_checkout_order(jsonb, jsonb, text, text) to service_role;
